...
export class BrowserWorkflows {
  ...
  async triggerResumeDownloads() {
    ...
    try {
      ...
      Logger.success('任務管理器(恢復下載功能) - 加載完成');
    } catch (error) {
      Logger.error('任務管理器(恢復下載功能) - 加載失敗', error);
    ...
    try {
      Logger.info('恢復下載 - 等待觸發');

      await page.evaluate(() => {
        return downloadInterface.StartAllDownloads('-1');
      });

      Logger.success('恢復下載 - 觸發成功');
    } catch (error) {
      Logger.error('恢復下載 - 觸發失敗', error);
    }
